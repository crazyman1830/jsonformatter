...
def validate_json(raw_json: str) -> Tuple[bool, str]:
    """
...
def format_json(raw_json: str, indent: int = 2) -> Dict[str, Union[bool, str]]:
    """
    ...
    if not is_valid:
        return {"success": False, "formatted_json": "", "error_message": error_message}
    ...
    try:
        ...
        return {"success": True, "formatted_json": formatted_json, "error_message": ""}

    except ValueError as e:
        error_msg = "JSON parsing error: {}".format(str(e))
        return {"success": False, "formatted_json": "", "error_message": error_msg}
    except Exception as e:
        return {
            "success": False,
            "formatted_json": "",
...
def parse_json_safely(raw_json: str) -> Tuple[Optional[Any], str]:
    """
...
class JSONProcessor(object):
    ...
    def __init__(self, default_indent: int = 2) -> None:
        """
    ...
    def format_json(
        self, raw_json: str, indent: Optional[int] = None
    ...
    def validate_json(self, raw_json: str) -> Dict[str, Union[bool, str]]:
        """
        ...
        """
        if raw_json is None:
            return {"is_valid": False, "error_message": "Input cannot be None"}

        if not isinstance(raw_json, str):
            return {"is_valid": False, "error_message": "Input must be a string"}

        is_valid, error_message = validate_json(raw_json)
        return {"is_valid": is_valid, "error_message": error_message}

    def parse_json(self, raw_json: str) -> Dict[str, Union[bool, Any, str]]:
        """
        ...
        if parsed_data is not None:
            return {"success": True, "data": parsed_data, "error_message": ""}
        else:
            return {"success": False, "data": None, "error_message": error_message}

    def get_json_info(
        self, raw_json: str
    ) -> Dict[str, Union[bool, Dict[str, Any], str]]:
        """
        ...
        return {"success": True, "info": info, "error_message": ""}
