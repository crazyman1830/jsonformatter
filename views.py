...
def format_json_endpoint() -> Tuple[Response, int]:
...
def validate_json_endpoint() -> Tuple[Response, int]:
    ...
    try:
        ...
        if not json_data:
            return (
                jsonify({"is_valid": False, "error_message": "No JSON data provided"}),
                400,
            )
        ...
        if not isinstance(json_data, str):
            return (
                jsonify(
                    {"is_valid": False, "error_message": "JSON data must be a string"}
                ),
                400,
            )
        ...
        return (
            jsonify({"is_valid": False, "error_message": f"Server error: {str(e)}"}),
            500,
        )


@api_bp.route("/comments", methods=["POST"])
def save_comments_endpoint() -> Tuple[Response, int]:
    """Save comments for JSON"""
    try:
        if not request.is_json:
            return (
                jsonify({"success": False, "error_message": "Request must be JSON"}),
                400,
            )
        ...
        if not isinstance(comments, str):
            return (
                jsonify(
                    {"success": False, "error_message": "Comments must be a string"}
                ),
                400,
            )
        ...
        return jsonify({"success": True, "message": "Comments saved successfully"}), 200

    except Exception as e:
        return (
            jsonify({"success": False, "error_message": f"Server error: {str(e)}"}),
            500,
        )


@api_bp.route("/comments", methods=["GET"])
def load_comments_endpoint() -> Tuple[Response, int]:
    """Load saved comments"""
    try:
